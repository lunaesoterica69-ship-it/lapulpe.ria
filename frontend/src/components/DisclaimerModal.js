...
// Disclaimer simple - se muestra CADA VEZ que se entra
const DisclaimerModal = ({ onClose }) => {
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div className="bg-stone-900 rounded-3xl border border-stone-700 max-w-md w-full shadow-2xl">
        {/* Header */}
        ...
        <div className="px-6 pb-6">
          <button
            onClick={onClose}
            className="w-full bg-red-600 hover:bg-red-500 text-white py-4 rounded-xl font-bold transition-colors text-lg"
          >
            Entendido, continuar
          </button>
        </div>
