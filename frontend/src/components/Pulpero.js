// Pulpero - La mascota que representa al dueño de la pulpería
// Los mensajes simulan al pulpero (dueño) hablándole al cliente

const Pulpero = ({ status, size = 'md' }) => {
  const sizes = {
    sm: 'w-16 h-16',
    md: 'w-24 h-24',
    lg: 'w-32 h-32'
  ...
  // Pending: Pulpero recibiendo el pedido
  if (status === 'pending') {
    return (
      <div className={`${sizes[size]} relative`}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          {/* Clipboard/list in tentacle */}
          <rect x="8" y="65" width="12" height="16" rx="2" fill="white" className="animate-float-up" />
          <line x1="10" y1="70" x2="18" y2="70" stroke="#991b1b" strokeWidth="1" />
          <line x1="10" y1="74" x2="16" y2="74" stroke="#991b1b" strokeWidth="1" />
          <line x1="10" y1="78" x2="17" y2="78" stroke="#991b1b" strokeWidth="1" />
        </svg>
        <style>{`
          ...
          @keyframes float-up { 0%, 100% { opacity: 0.8; transform: translateY(0); } 50% { opacity: 1; transform: translateY(-5px); } }
  ...
  // Accepted: Pulpero preparando el pedido
  if (status === 'accepted') {
    return (
      <div className={`${sizes[size]} relative`}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          {/* Apron/delantal */}
          <path d="M30 55 L50 60 L70 55 L65 85 L35 85 Z" fill="#dc2626" />
          <rect x="42" y="62" width="16" height="10" rx="2" fill="#fef2f2" />
          ...
          {/* Shopping bag in tentacle */}
          <g className="animate-sparkle">
            <rect x="75" y="30" width="14" height="18" rx="2" fill="#fbbf24" />
            <path d="M78 30 Q82 25 86 30" stroke="#fbbf24" strokeWidth="2" fill="none" />
          </g>
        </svg>
        <style>{`
          ...
          @keyframes sparkle { 0%, 100% { opacity: 0.8; transform: scale(0.95); } 50% { opacity: 1; transform: scale(1.05); } }
  ...
  // Ready: Pulpero celebrando - pedido listo!
  if (status === 'ready') {
    return (
      <div className={`${sizes[size]} relative`}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          <g className="animate-ring">
            ...
            <path d="M82 12 L88 12 L88 18 L82 18 Z" fill="#f59e0b" />
            <circle cx="85" cy="20" r="2" fill="#92400e" />
          </g>
        </svg>
        <style>{`
          ...
          .animate-ring { animation: ring 0.3s ease-in-out infinite; transform-origin: center top; }
        `}</style>
      </div>
    );
  }

  // Completed: Pulpero satisfecho
  return (
    <div className={`${sizes[size]} relative`}>
      <svg viewBox="0 0 100 100" className="w-full h-full opacity-60">
        ...
        {/* Checkmark */}
        <circle cx="75" cy="20" r="10" fill="#22c55e" opacity="0.8" />
        <path d="M70 20 L74 24 L82 16" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" />
      </svg>
    </div>
  );
};

// Mensajes del Pulpero (dueño de la pulpería hablándole al cliente)
// Estos mensajes simulan a una persona real atendiendo
export const getPulperoMessage = (status) => {
  const messages = {
    pending: [
      "¡Hola! Ya recibí tu pedido, déjame revisarlo...",
      "¡Qué bueno verte! Dame un momentito...",
      "¡Recibido! Ya lo estoy buscando...",
      "Un segundito que te organizo todo..."
    ],
    accepted: [
      "¡Ya estoy en eso! Preparando tu pedido con cariño...",
      "Todo va bien, ya casi lo tengo listo...",
      "Empacando tus cositas, ya merito...",
      "¡Trabajando para vos! Un ratito más..."
    ],
    ready: [
      "¡Listo compa! Ya podés pasar a buscarlo",
      "¡Todo preparado! Te estoy esperando",
      "¡Ya está! Vení cuando gustés",
      "¡Pedido listo! Aquí te lo tengo"
    ]
  };
  
  const statusMessages = messages[status];
  if (!statusMessages) return '';
  // Use a seed based on current minute to keep message consistent
